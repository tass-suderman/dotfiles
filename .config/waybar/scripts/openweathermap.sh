#!/bin/bash
#
# Not my own work. Credit to original author (who, despite my best effort, I cannot find to credit)

# Configuration
API_KEY="YOUR_API_KEY"
CITY_ID="618323" # Saskatoon city ID
UNITS="metric"
LANG="en"

# Function to check if API key is valid
check_api_key() {
  local test_request=$(curl -s "api.openweathermap.org/data/2.5/weather?q=Saskatoon&appid=$API_KEY")
...
# Get weather data
WEATHER_DATA=$(curl -s "api.openweathermap.org/data/2.5/weather?q=Saskatoon&appid=$API_KEY&units=$UNITS&lang=$LANG")
